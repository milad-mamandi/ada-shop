    ...
    return (
        <div className='mt-8 flex flex-col'>
            <div className='flex flex-col gap-4'>
                {cart.map(i => (
                    <div className='flex flex-row justify-between rounded-lg border-[3px] border-purple-700 p-4 shadow-md'>
                ...
                {cart.length < 1 && (
                    <div className='mt-8 flex items-center justify-center text-2xl'>
                    ...
                    <div className='mt-8 flex flex-col justify-between gap-4 md:flex-row'>
                        <div className='flex flex-col gap-2'>
                            <p className='text-fuchsia-700'>
                                تکمیل و ادامه فرایند پرداخت {calTotalProducts(cart)} کالا از {cart.length} محصول انتخاب
                                شده
                            </p>
                            <p className='text-sm'>
                                مبلغ پرداخت سفارش{' '}
                                <span className='font-bold'>{priceSeperator(calTotalPrice(cart, mockProducts))}</span>{' '}
                                تومان
                            </p>
                        </div>
                        <Dialog>
